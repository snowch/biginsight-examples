...
if __name__ == "__main__":
    if len(sys.argv) != 9:
        print("Usage: exporttoswift <hdfs file> <auth url> <tenant> <username> <password> <region> <auth method> <container>", file=sys.stderr)
        exit(-1)
    ...
    # read file from HDFS
    lines = sc.textFile(hdfs_filename, 1)
    
    src = sqlContext.read.format('com.databricks.spark.csv').options(header='false', inferschema='true').load(hdfs_filename)
    ...
    # save to swift
    src.rdd.saveAsTextFile(swift_file_url)
